...
app.config['MAX_CONTENT_LENGTH'] = 16 * 1024 * 1024  # 16 MB (imágenes)
...
def get_db():
    ...
    try:
        ...
        return psycopg2.connect(
            host="postgres", database="familychancay",
            user="chancay", password="chancay_123",
...
def columnas_de(tabla):
    """Devuelve la lista de columnas que tiene una tabla (para armar queries a prueba de cambios)."""
    conn = get_db()
    if not conn: return []
    cur = conn.cursor()
    try:
...
def init_schema():
    ...
    try:
        ...
        print(f"⚠️ init_schema: {e}"); conn.rollback()
    finally:
        cur.close(); conn.close()
...
def get_productos():
    ...
    if categoria and categoria != 'Todos':
        query += " AND LOWER(c.nombre) = LOWER(%s)"; params.append(categoria)
    if busqueda:
        query += " AND (p.nombre ILIKE %s OR p.codigo_producto ILIKE %s)"
        params.extend([f"%{busqueda}%", f"%{busqueda}%"])
    query += " ORDER BY p.producto_id ASC LIMIT 200"
    try:
        cur.execute(query, params); return jsonify(serializar(cur.fetchall()))
    except Exception as e:
        print(f"Error Productos: {e}"); return jsonify({"error": str(e)}), 500
    finally:
        cur.close(); conn.close()
...
def get_producto(pid):
    ...
    try:
        cur.execute("""
            ...
            JOIN marcas m ON p.marca_id=m.marca_id WHERE p.producto_id=%s
        """, (pid,))
        p = cur.fetchone()
        if not p: return jsonify({"error": "No existe"}), 404
        cols = columnas_de('producto_variantes')
        v = None
        ...
        try:
            ...
            r = cur.fetchone()
            if r and r['stock_actual'] is not None: stock = int(r['stock_actual'])
        except Exception:
            pass
        d = serializar([p])[0]
...
def crear_producto():
    ...
    cat_id = int(d.get('categoria_id') or 1); mar_id = int(d.get('marca_id') or 1)
    precio = float(d.get('precio') or 0)
    talla = (d.get('talla') or '').strip() or 'U'; color = (d.get('color') or '').strip() or 'N/A'
    stock = int(d.get('stock') or 0); imagen = d.get('imagen') or None; desc = d.get('descripcion') or ''
    cur = conn.cursor()
    try:
        ...
        conn.rollback(); return jsonify({"error": f"El código '{codigo}' ya existe. Usa otro."}), 400
    except Exception as e:
        conn.rollback(); print(f"Error crear: {e}"); return jsonify({"error": str(e)}), 500
    finally:
        cur.close(); conn.close()
...
def actualizar_producto(pid):
    conn = get_db()
    if not conn: return jsonify({"error": "DB Down"}), 503
    d = request.json or {}; cur = conn.cursor()
    try:
        sets, params = [], []
        ...
        if sets:
            params.append(pid); cur.execute(f"UPDATE productos SET {', '.join(sets)} WHERE producto_id=%s", params)
        talla = d.get('talla'); color = d.get('color'); precio = d.get('precio'); vid = None
        if 'producto_id' in columnas_de('producto_variantes'):
            ...
            if vr:
                ...
                if vsets:
                    vparams.append(vr['variante_id']); cur.execute(f"UPDATE producto_variantes SET {', '.join(vsets)} WHERE variante_id=%s", vparams)
                vid = vr['variante_id']
        if 'stock' in d: actualizar_inventario(cur, pid, vid, int(d['stock']))
        conn.commit(); return jsonify({"status": "ok"})
    except Exception as e:
        conn.rollback(); print(f"Error update: {e}"); return jsonify({"error": str(e)}), 500
    finally:
        cur.close(); conn.close()
...
def get_ventas():
    ...
    query = """SELECT v.numero_venta, v.fecha_venta, v.total, v.estado_venta,
               cl.nombres || ' ' || cl.apellidos as cliente, e.nombres || ' ' || e.apellidos as vendedor,
               mp.nombre_metodo as metodo_pago
        FROM ventas v JOIN clientes cl ON v.cliente_id = cl.cliente_id
        JOIN empleados e ON v.empleado_id = e.empleado_id JOIN metodos_pago mp ON v.metodo_pago_id = mp.metodo_pago_id
        WHERE v.estado_venta = 'COMPLETADA'"""
    params = []
    if busqueda:
        query += " AND (v.numero_venta ILIKE %s OR cl.nombres ILIKE %s)"; params.extend([f"%{busqueda}%", f"%{busqueda}%"])
...
def get_empleados():
    ...
    try:
        cur.execute("""SELECT e.codigo_empleado, e.nombres, e.apellidos, r.nombre_rol as rol,
               s.nombre as sucursal, e.telefono, e.correo FROM empleados e
               JOIN roles r ON e.rol_id=r.rol_id JOIN sucursales s ON e.sucursal_id=s.sucursal_id
...
# >>> RANKING AUTO‑DETECTIVO: funciona con producto_id O con variante_id <<<
...
def get_top_productos():
    ...
    if not cols:
        return jsonify([])  # tabla no existe aún → lista vacía limpia (sin error 500)
    cur = conn.cursor()
    try:
        if 'producto_id' in cols:
            cur.execute("""SELECT p.nombre as producto, c.nombre as categoria, m.nombre as marca,
                   ...
                   FROM detalle_ventas dv JOIN productos p ON dv.producto_id=p.producto_id
                   JOIN categorias c ON p.categoria_id=c.categoria_id JOIN marcas m ON p.marca_id=m.marca_id
                   GROUP BY p.nombre,c.nombre,m.nombre ORDER BY unidades_vendidas DESC LIMIT 10""")
        elif 'variante_id' in cols:
            cur.execute("""SELECT p.nombre as producto, c.nombre as categoria, m.nombre as marca,
                   SUM(dv.cantidad) as unidades_vendidas, SUM(dv.subtotal) as total_generado
                   FROM detalle_ventas dv JOIN producto_variantes pv ON dv.variante_id=pv.variante_id
        ...
        print(f"Error Top Productos: {e}"); return jsonify({"error": str(e)}), 500
    finally:
